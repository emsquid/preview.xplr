...
function previewManager.save_preview(preview, node, ctx)
    ...
    -- add needed data
    if type == "text" then
        -- TODO: add text relative position
...
function previewManager.load.text(file, ctx, args)
    local preview = ""
...
function previewManager.load.image(image, ctx, args)
    ...
    if args.image.method == "kitty" then
        ...
        local success = helper.load_image(path, id, width, height)
...
function previewManager.load.directory(directory, ctx, args)
    local preview = ""
...
function previewManager.load.other(node, ctx, args)
...
function previewManager.preview(node, ctx, args)
    ...
    if args.image.method == "kitty" and path ~= previewManager.current.path then
        if instance.type == "image" then
            helper.display_image(instance.id, ctx.layout_size.x + 1, ctx.layout_size.y + 1)
...
function previewManager.handle(node, ctx, args)
    if previewManager.should_reload_preview(node, ctx) then
        previewManager.load[helper.type(node)](node, ctx, args)
    end
